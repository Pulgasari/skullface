// @skullface/core/modules/paths.js

import { DEFAULT_APP_NAME } from './defaults.js';

function normalizedPlatform (name) {
  return {
    android : 'android',
    darwin  : 'mac',
    freebsd : 'freebsd',
...
const platform = normalizedPlatform(Deno.build.os);
...
export default function getPaths (appName = DEFAULT_APP_NAME) {
  ...
  const paths = {
    ...
    // Utils
    join (...parts) { return joinPaths(parts); },
    async ensureExists (path) { await ensurePathExists(path); }
  };
